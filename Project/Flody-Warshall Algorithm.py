...
def Solution(file_path, source, destination, bandwidth_requirement):
    ...
    # Find the shortest path using Dijkstra's algorithm
    distance, path = dijkstra_algorithm(G, source, destination)

    # Using Floyd-Warshall to compute all-pairs shortest paths
    all_pairs_shortest_paths = floyd_warshall_algorithm(adjacency_matrix)
    # Retrieve distance between source and destination from Floyd-Warshall matrix
    fw_distance = all_pairs_shortest_paths[source][destination]
